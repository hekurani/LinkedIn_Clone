...
export class RepostsService {
  constructor(
    @InjectRepository(Reposts) private repostRepository: Repository<Reposts>,
    private usersService: UsersService,
    private postService: PostsService,
  ) {}
  async createRepost(userId: number, postId: number) {
    const reposts = await this.repostRepository.find({
      where: { user: { id: userId }, posts: { id: postId } },
    });
    if (reposts.length)
      throw new ForbiddenException('This repost already exists!');

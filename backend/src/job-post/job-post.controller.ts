...
@Post()
createJobPost(@Body() createJobPostDto: CreateJobPostDto,@AuthUser() user: {userId:number}){
...
@Patch('/:id')
updateJobPost(@Body() updateJobPostDto: UpdateJobPostDto,@Param('id') id: number,@AuthUser() user: {userId:number}){
return this.jobPostService.updateJobPost(updateJobPostDto,id,user?.userId);
}

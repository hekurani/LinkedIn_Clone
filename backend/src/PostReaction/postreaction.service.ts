...
export class ReactionService {
   ...
   async createReaction(userId:number,postId:number,reactionStatus:ReactionStatus){
...
await this.reactionRepository.save(reactionPost);
   ...
   async deleteReaction(userId:number,postId:number){
    ...
    if(!postReaction) throw new NotFoundException("The post or user doesnt exists!");
...
return{
  ...
  await this.reactionRepository.update(
   ...
   const postReaction = await this.reactionRepository.findOne({ where: { user:{id:userId}, post:{id:postId} } });
   if(!postReaction) throw new NotFoundException("The post or user doesnt exists!");
    return {
        status: 'success',
        postReaction 

...
export class CompanyService {
  async  updateCompany(id: number, updateCompany: UpdateCommpanyDto) {
        ...
        if(updateCompany?.cityId) { city=await this.cityRepository.findOne({ where: { id: parseInt(updateCompany.cityId) } })
...
// return {status:'success',company}
   ...
   async createCompany(createCompanyDto:CreateCommpanyDto,userId:number){
    ...
    const city=await this.cityRepository.findOne({ where: { id: parseInt(createCompanyDto.cityId) } })
    if(!user) throw new NotFoundException("No user exist!");
    if(!city) throw new NotFoundException("No city with given id exist!");
    const company= this.companyRepository.create({...createCompanyDto,cityId:city,owner:user});
...
async  getUser(email:string,password:string):Promise<User>{ const [user] = await this.userService.findByPassword(email,false);
...
return company;
    ...
    async deleteCompany(id:number,email:string,password:string,userId:number){
       const user=this.getUser(email,password)
            if((await user).id!==userId) throw new BadRequestException("You havent entered right creditials, please try again!"); 
            
            const company = await this.companyRepository.findOne({ where: { id, owner:await user } });
    
    if (!company) {
      throw new NotFoundException("Company not found or you do not have permission to delete it");
    }

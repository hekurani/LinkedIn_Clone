...
constructor(@InjectRepository(country) private CountryRepository: Repository<country>,@InjectRepository(city) private CityRepository: Repository<city>){}
   async createCountry(country:String){
    ...
    async createCity(city:String,countryId:number){
        ...
        if(!country) throw new NotFoundException("There's no country valid with this id!")
...
if(!country) throw new NotFoundException("There's no country valid with this id!")
   const cities=await this.CityRepository.find({where:{country:{id}}})
return cities;
}

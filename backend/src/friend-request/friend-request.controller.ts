...
return this.FriendRequestService.getSendedRequestsByUserId(user?.userId);
    ...
    @Post(':id/accept')
    async acceptFriendRequest(@AuthUser() user: {userId:number}, @Param('id') id: number) {
   ...
   return this.FriendRequestService.rejectRequest(user,id);
    ...
    @Get('user/:id')
    async getFriendRequestsByUserId(@AuthUser() user: {userId:number}, @Param('id') userId: number) {
        return this.FriendRequestService.getFriendRequestsByUserId(user?.userId,userId);
    }
    @Post('user/:id/block')
    async blockFriendRequest(@AuthUser() user:{userId:number},@Param('id') userId:number){
return this.FriendRequestService.blockFriendRequest(user?.userId,userId);
    }
    @Post('user/:id/unblock')
    async unBlockFriendRequest(@AuthUser() user:{userId:number},@Param('id') userId:number){

...
export class FriendRequestService {
    ...
    private readonly friendRequestRepository: Repository<FriendRequest>,@InjectRepository(Friend)
    private readonly friendRepository: Repository<Friend>,private readonly usersService:UsersService){}
...
if(user.id===id){
    throw new ForbiddenException("You are not allowed to send request to yourself");
}
...
if(exitsFriend){
throw new ForbiddenException("You are friends, you cant make an request!")
}
...
return {
   ...
   async acceptRequest(userId:number,id:number):Promise<{
    friend:Friend,
    ...
    if(!id){
        throw new NotFoundException("The request that you want to accept is not defined");
    }
const friendRequest=await this.friendRequestRepository.findOne({
        where:{
            id:id
    ...
    console.log("friend request = ",friendRequest,friendRequest.receiver);
    if(!friendRequest){
    ...
    async cancelRequest(user:User,id:number):Promise<{status:String}> {
        ...
        if(!id){
            throw new NotFoundException("The request that you want to cancel is not defined");
        }
        const friendRequest=await this.friendRequestRepository.findOne({
            where:{
                id:id
    ...
    if(friendRequest.status!=="pending"){
        throw new ForbiddenException("You cant cancel an request thats not in pending state")
    }
    ...
    if(friendRequest.status!=="pending"){
        throw new ForbiddenException("You cant reject an request thats not in pending state")
    }
    if(friendRequest.sender===user){
    ...
    if(friendRequest.receiver!==user){
        throw new ForbiddenException("Ypu cant reject an request if you didnt send it")
    }
...
receiver:{
    id:userId
        ...
        if(!friendRequest){
            ...
            const friendRquest=await this.friendRequestRepository.save(friendRequest);
            return;
        }
